...
const makeTask = ({
  ...
  routineName = "Routine",
  expSeconds = 60,
}: MakeArgs): ScheduledTaskWithExpectedStartTime => ({
  id,
  activity: {
    id,
...
const kinds = (items: ScheduledVisibleItem[]) => items.map((i) => i.kind);

const wrappedAt = (
  items: ScheduledVisibleItem[],
  index: number,
): WrappedRoutineItem => {
...
describe("groupRoutineTasks — J. Edge cases & invariants", () => {
  ...
  it("J5 variant: never produces two cards for the same routine_application_id", () => {
    ...
    const result = groupRoutineTasks(tasks);
    const ids = result
      .filter((i): i is WrappedRoutineItem => i.kind === "wrap")
