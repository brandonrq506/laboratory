import {
  MAX_ABSORBED_FOREIGN,
  MIN_TASKS_TO_WRAP,
  WRAP_POINT,
  WRAP_TOLERANCE,
} from "@/constants/routine-wrap";
import type {
  PlainScheduledItem,
...
const collectRoutineGroups = (
  tasks: readonly ScheduledTaskWithExpectedStartTime[],
...
/**
 * Rule 2: first member is far enough down the visible list that hiding it
 * doesn't obscure what the user is about to do.
 */
const meetsPositionThreshold = (firstMemberVisiblePosition: number) =>
  firstMemberVisiblePosition >= WRAP_POINT;
...
const qualifiesForWrap = (
  group: RoutineGroup,
  firstMemberVisiblePosition: number,
): boolean =>
  meetsMinimumSize(group) &&
  meetsPositionThreshold(firstMemberVisiblePosition) &&
  meetsGapTolerance(group) &&
  meetsAbsorptionCap(group);

/**
 * Wrap a single scheduled task in the visible-item discriminated union so it
...
/**
 * Produce the rendered list ("visible items") from the canonical scheduled
 * list. Walks tasks left-to-right; at each cursor position, if a routine
 * group starts there AND qualifies for wrapping at the current visible
 * position, emit one wrapped card and skip past its span. Otherwise emit a
 * plain task and advance by one.
 *
 * "Visible position" = 1-indexed position in the OUTPUT list (a wrapped card
 ...
 */
const projectVisibleItems = (
  tasks: readonly ScheduledTaskWithExpectedStartTime[],
  groupsByFirstMemberIndex: ReadonlyMap<number, RoutineGroup>,
): ScheduledVisibleItem[] => {
  const items: ScheduledVisibleItem[] = [];
  let cursor = 0;

  while (cursor < tasks.length) {
    const group = groupsByFirstMemberIndex.get(cursor);
    const nextVisiblePosition = items.length + 1;

    if (group && qualifiesForWrap(group, nextVisiblePosition)) {
...
/**
 * Transform a flat scheduled-task list into the visible-card list rendered by
...
export const groupRoutineTasks = (
  ...
  return projectVisibleItems(tasks, groupsByFirstMember);
};
